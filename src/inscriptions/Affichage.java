...
public class Affichage {
	...
	public Option optionsCompetition(String nom, Inscriptions inscriptions) {
		...
		options.add(
				"Ajouter une " + (compTmp.estEnEquipe() ? "une équipe" : "une personne") + " parmis celles inscrites");
				...
				new ListData<String>() {
					...
					public void itemSelected(int index, String someone) {
						...
						case 1:
							...
							System.out.println(nbr + " : Annuler");
							int choix = InOut.getInt("Taper le numéro de "
									+ (compTmp.estEnEquipe() ? "l'équipe" : "la personne") + " à supprimer : ");
							nbr = 0;
							for (Candidat c : compTmp.getCandidats()) {
								if (choix == nbr) {
						...
						case 2:
							System.out.println(
									"Liste des " + (compTmp.estEnEquipe() ? "équipes" : "participants") + " :");
